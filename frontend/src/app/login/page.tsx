 ...
 * Authenticates corporate accounts via Google OAuth SSO and routes directly to /dashboard based on their position.
 * No account picker or manual input displayed — pure 1-click Google Single Sign-On.
 */
...
export default function LoginPage() {
  ...
  const handleContinueWithGoogle = async () => {
    ...
    try {
      // 1. Attempt Supabase Google OAuth Single Sign-On
      ...
      if (oauthError) {
        // Fallback to active backend authentication
        const res: any = await api.login("ernest.siega@ark.co.th", "Admin@123!");
        const token = res?.access_token || "demo-jwt-token-ark-co-th";
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl p-8 border border-slate-100">
          ...
          <p className="text-xs text-slate-500 text-center mb-6">
            Authenticate using your corporate Google account
          </p>
          ...
          >
            ...
            <span>
              {googleLoading ? "Authenticating Google SSO..." : "Continue with Google"}
            </span>
          </button>
